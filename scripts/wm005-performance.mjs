...
const result={method:'Four alternating-order same-browser/runtime repetitions; default adaptive quality; no recorder, fixture placement or competing renderer; actual ordinary practice forward/back input. Criterion: measured mean FPS >=30, or the narrow software-renderer baseline exception; mean FPS loss <=10%. Real-browser30FPS target remains; software exception only when exact accepted baseline also fails.',records,comparisons,status:comparisons.every(x=>x.pass)?'PASS':'NOT_PASS'};
