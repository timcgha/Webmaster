...
process.env.WM_SOURCE=execFileSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).trim();
process.env.WM_TREE=execFileSync('git',['rev-parse','HEAD^{tree}'],{encoding:'utf8'}).trim();
...
for(const n of ['wm001','wm002','wm003']){
 ...
 if(n==='wm002'){
  ...
  assert.ok(a>=0&&b>a,'recognized original keyboard route boundary');
  s=s.slice(0,a)+`async function keyboardRoute(page:Page, _captures=true){await currentLegacyRoute(page,false);await stage(page,4);}\n`+s.slice(b);
  const c=s.indexOf('async function controllerRoute('),d=s.indexOf('test("WM-002 simulated',c);
  assert.ok(c>=0&&d>c,'recognized original controller route boundary');
