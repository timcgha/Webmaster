...
const root=path.resolve('.wm005-pages'),out=path.resolve('.wm005-payload'),base='/Webmaster/wm005-preview/';
...
for(const f of['preview.mjs','preview-storage.mjs','preview.css'])fs.writeFileSync(path.join(out,f),fs.readFileSync(`preview/wm005/${f}`,'utf8').replace('__GAME_ENTRY__',entry.file));
...
const provenance={change:'WM-005',kind:'isolated sponsor playtest preview',source:git('rev-parse','HEAD'),tree:git('rev-parse','HEAD^{tree}'),base,saveNamespace:'webmaster.wm005-preview.v1:',build:'clean Vite8.3.0 / exact pnpm lock / Node24.19.0',payloadSha256:digest,hashMethod:'SHA256(sorted UTF8 path NUL SHA256 NUL byteLength LF), excluding provenance and manifest',runtimeFiles:rows.length,rawBytes:rows.reduce((s,x)=>s+x.bytes,0),gzip9Bytes:rows.reduce((s,x)=>s+x.gzip9,0),priorRoot:'b4a583116fc0823434f11788e5ab0bec36358f4d',mergeOrRootRelease:false};
...
fs.mkdirSync('evidence/wm-005',{recursive:true});fs.writeFileSync('evidence/wm-005/packaging.json',JSON.stringify({provenance,files:rows,largest:[...rows].sort((a,b)=>b.bytes-a.bytes).slice(0,5)},null,2));console.log(JSON.stringify(provenance));
