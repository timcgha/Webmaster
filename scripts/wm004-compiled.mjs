...
const base='/Webmaster/wm004-preview/',url='http://127.0.0.1:4177'+base+'?test=1&diagnostics=1',prefix='webmaster.wm004-preview.v1:';
const out=path.resolve('.wm004-payload'),reportDir='evidence/wm-004/compiled';fs.mkdirSync(reportDir,{recursive:true});
const git=(root,...args)=>execFileSync('git',args,{cwd:root,encoding:'utf8'}).trim();
const baseline=path.resolve('../baseline'),derived=path.resolve('.wm004-derived');fs.mkdirSync(derived,{recursive:true});
...
const payload={...old[1].payload,position:{x:-11,y:2.6,z:-51},climb:{version:1,checkpoint:6,completed:true},updatedAt:Date.now()-86400000};
...
try{for(const fixture of[...old,{name:'new-street',stored:{}}]){
  ...
  try{
    ...
    if(fixture.name==='new-street'){
      ...
      assert.deepEqual((await page.evaluate(()=>window.__WM_DEBUG__.getState())).position,fixture.payload.position);
    ...
    await page.goto(url);await page.locator('#loading.hidden').waitFor({timeout:45000});assert.deepEqual(await nativeState(page),saved);
    await page.getByRole('button',{name:/Continue/}).click();await page.waitForFunction(()=>window.__WM_DEBUG__.getState().safe);
    ...
    await page.screenshot({path:`${reportDir}/${fixture.name}-reopened.png`});results.push({fixture:fixture.name,status:'PASS',method:'Compiled isolated preview; genuine menu actions; exact old writer fixture; Save Game/Save & Quit; Chromium process closes and reopens same persistent profile without reinjection; identical stored generations and preserved root/WM003 keys',loaded,reopened,errors});
  }finally{await ctx.close();}
}}finally{await new Promise(resolve=>server.close(resolve));fs.writeFileSync(reportDir+'/results.json',JSON.stringify({source:git('.','rev-parse','HEAD'),tree:git('.','rev-parse','HEAD^{tree}'),results,complete:results.length===4},null,2));}
assert.equal(results.length,4);console.log(JSON.stringify({status:'PASS',cases:results.map(x=>x.fixture)}));
