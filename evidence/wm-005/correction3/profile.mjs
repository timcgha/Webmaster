...
try{
  ...
  for(const[width,height]of[[1280,720]])for(const round of[0])for(const name of round%2?['candidate','baseline']:['baseline','candidate']){
    ...
    try{
      ...
      const cdp=await context.newCDPSession(page);await cdp.send('Profiler.enable');await cdp.send('Profiler.start');await cdp.send('Performance.enable');const before=await cdp.send('Performance.getMetrics');
      const {times,states}=await sample(page,phase),buckets=Array.from({length:12},(_,i)=>times.filter(t=>t>=i*1000&&t<(i+1)*1000).length);
      const after=await cdp.send('Performance.getMetrics'),{profile}=await cdp.send('Profiler.stop');fs.writeFileSync(`evidence/wm-005/diagnostic-${name}-cpu.json`,JSON.stringify({profile,before,after}));await cdp.detach();
...
fs.writeFileSync('evidence/wm-005/diagnostic.json',JSON.stringify({status:'DIAGNOSTIC_ONLY_NOT_ACCEPTANCE',records},null,2));
