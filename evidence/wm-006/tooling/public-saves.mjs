...
const base='/Webmaster/wm006-preview/',url='https://timcgha.github.io'+base+'?test=1&diagnostics=1',prefix='webmaster.wm006-preview.v1:';
const out=path.resolve('.wm006-payload'),reportDir='evidence/wm-006/public/saves';fs.mkdirSync(reportDir,{recursive:true});
const git=(root,...args)=>execFileSync('git',args,{cwd:root,encoding:'utf8'}).trim();
const baseline=path.resolve('../save-baseline'),derived=path.resolve('.wm006-derived');fs.mkdirSync(derived,{recursive:true});
...
const wm004Root=path.resolve('../baseline4'),wm004Derived=path.join(derived,'wm004');fs.mkdirSync(wm004Derived,{recursive:true});
...
const wm005Root=path.resolve('../baseline'),wm005Derived=path.join(derived,'wm005');fs.mkdirSync(wm005Derived,{recursive:true});
assert.equal(git(wm005Root,'rev-parse','HEAD'),'6aff5802736b5c12380102a70a38794838397d7d');
for(const file of fs.readdirSync(wm005Root+'/src/core').filter(x=>x.endsWith('.ts'))){
 const b=fs.readFileSync(wm005Root+'/src/core/'+file),blob=crypto.createHash('sha1').update('blob '+b.length+'\0').update(b).digest('hex');assert.equal(blob,git(wm005Root,'rev-parse','HEAD:src/core/'+file));
 fs.writeFileSync(wm005Derived+'/'+file.replace(/\.ts$/,'.mjs'),stripTypeScriptTypes(b.toString(),{mode:'transform'}).replace(/(from\s*["'])(\.[^"']+)(["'])/g,(_,a,p,z)=>a+p+'.mjs'+z));
}
const {SaveStore:WM005SaveStore}=await import(pathToFileURL(wm005Derived+'/save.mjs').href);
const fiveMap=new Map(),fiveMemory={getItem:k=>fiveMap.get(k)??null,setItem:(k,v)=>fiveMap.set(k,v),removeItem:k=>fiveMap.delete(k)};
const fivePayload={...payload,position:{x:-38,y:-18,z:-54},course:{version:1,next:20,completed:true}};
assert.equal(new WM005SaveStore(fiveMemory).write(1,'manual',fivePayload).ok,true);
old.push({name:'WM-005-street-completed',payload:fivePayload,stored:Object.fromEntries(fiveMap),source:git(wm005Root,'rev-parse','HEAD')});
...
try{for(const fixture of[...old,{name:'new-street',stored:{}}]){
  const profile=path.resolve(`.wm006-profiles/${fixture.name}`);fs.rmSync(profile,{recursive:true,force:true});
...
}}finally{await new Promise(resolve=>server.close(resolve));fs.writeFileSync(reportDir+'/results.json',JSON.stringify({source:git('.','rev-parse','HEAD'),tree:git('.','rev-parse','HEAD^{tree}'),results,failures,complete:results.length===7&&failures.length===0},null,2));}
assert.equal(results.length,7);console.log(JSON.stringify({status:'PASS',cases:results.map(x=>x.fixture)}));
