...
const records=[];let browser;const out='evidence/wm-006/performance-diagnosis';fs.mkdirSync(out,{recursive:true});
...
try{
  ...
  for(const[width,height]of[[1920,1080]])for(const round of[0,1])for(const name of round%2?['candidate','baseline']:['baseline','candidate']){
    ...
    try{
      ...
      for(const mode of ['native','lean-observer','hidden-hud','idle']){
...
fs.writeFileSync(out+'/summary.json',JSON.stringify({classification:'DIAGNOSTIC_ONLY, not a gate pass; hidden-hud condition is not eligible acceptance evidence',source:git(roots.candidate,'rev-parse','HEAD'),tree:git(roots.candidate,'rev-parse','HEAD^{tree}'),records:records.map(({frameTimesMs,states,...r})=>({...r,first:states[0],last:states.at(-1)}))},null,2));
