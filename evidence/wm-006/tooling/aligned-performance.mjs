...
const records=[];let browser;const out='evidence/wm-006/aligned-performance';fs.mkdirSync(out,{recursive:true});
...
try{
  ...
  for(const[width,height]of[[1280,720],[1920,1080]])for(const round of[0,1,2,3])for(const name of round%2?['candidate','baseline']:['baseline','candidate']){
    ...
    try{
      ...
      const setup=await page.evaluate(()=>{const c=document.querySelector('#game-canvas');return{state:window.__wm005Controls.state(),hero:window.__WM_DEBUG__.getHeroView(),canvas:[c.width,c.height]};});
      ...
      fs.writeFileSync(out+'/partial.json',JSON.stringify({status:'INCOMPLETE',records},null,2));
      console.log(JSON.stringify({...records.at(-1),frameTimesMs:undefined,states:undefined,setup:undefined}));
      await page.screenshot({path:`${out}/${name}-${width}-${round}.png`});
      }
    }finally{await context.close();await server.close();}
  }
}finally{await browser?.close();}
const comparisons=[];
for(const width of[1280,1920])for(const phase of ['combat-street']){
...
const result={method:'C2 unchanged source. Four alternating-order paired combat-street rounds at each resolution against exact merged WM005, same original sampling and repeated punch/kick/web input. Repair only baseline approach heading: ordinary forward input aligns north-facing hero to the candidate rather than leaving WM005 side-on. Assert heading, camera angles and street location before sampling; record actual canvas, state and hero geometry. No product, fixture, time, geometry, resolution or threshold changes. 30FPS target and narrow software-baseline exception, no material regression greater than10%, unchanged. Prior failed run remains failed. Other six unchanged-source legacy cells retain their original C2 evidence.',records,comparisons,status:comparisons.every(x=>x.pass)?'PASS':'NOT_PASS'};
fs.writeFileSync(out+'/performance.json',JSON.stringify(result,null,2));console.log(JSON.stringify(comparisons));assert.equal(result.status,'PASS');
