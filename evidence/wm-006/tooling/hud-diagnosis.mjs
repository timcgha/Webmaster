...
const records=[];let browser;const out='evidence/wm-006/hud-diagnosis';fs.mkdirSync(out,{recursive:true});
...
async function sample(page,phase,mode){
 return page.evaluate(async ({phase,mode})=>{
  ...
  while(true){await new Promise(requestAnimationFrame);const now=performance.now();start??=now;const t=now-start;times.push(t);
   ...
   if(t>=12000)break;
   if(mode==='native')active=!!read().combat?.active;
...
try{
  ...
  for(const[width,height]of[[1920,1080]])for(const round of[0])for(const name of round%2?['candidate','baseline']:['baseline','candidate']){
    ...
    try{
      ...
      for(const mode of ['native','frozen-hud']){
      await page.evaluate(()=>window.__wm005Controls.rest(1500));
      if(mode==='frozen-hud')await page.evaluate(()=>{const descriptor=Object.getOwnPropertyDescriptor(Node.prototype,'textContent');window.__hudTextDescriptor=descriptor;Object.defineProperty(Node.prototype,'textContent',{...descriptor,set(value){if(this instanceof Element&&this.closest('#hud-layer'))return;descriptor.set.call(this,value);}});});
      ...
      await page.evaluate(()=>{document.querySelector('#diagnostic-only-style')?.remove();if(window.__hudTextDescriptor){Object.defineProperty(Node.prototype,'textContent',window.__hudTextDescriptor);delete window.__hudTextDescriptor;}});
      }
      }
    }finally{await context.close();await server.close();}
  }
}finally{await browser?.close();}
fs.writeFileSync(out+'/summary.json',JSON.stringify({classification:'DIAGNOSTIC_ONLY, not a gate pass; frozen-hud condition is not eligible acceptance evidence',source:git(roots.candidate,'rev-parse','HEAD'),tree:git(roots.candidate,'rev-parse','HEAD^{tree}'),records:records.map(({frameTimesMs,states,...r})=>({...r,first:states[0],last:states.at(-1)}))},null,2));
