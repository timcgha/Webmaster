...
const base='/Webmaster/',url=(process.env.WM_PUBLIC_ORIGIN||'http://127.0.0.1:4177')+base+'?test=1',prefix='';
const out=path.resolve('.wm004-payload'),reportDir='evidence/wm-004/root-release/'+(process.env.WM_PUBLIC_ORIGIN?'public':'prepared');fs.mkdirSync(reportDir,{recursive:true});
...
try{for(const fixture of[...old,{name:'new-street',stored:{}}]){
  ...
  try{
    ...
    const saved=await nativeState(page),rootOnly=x=>Object.fromEntries(Object.entries(x).filter(([k])=>k.startsWith('webmaster.wm003-preview.v1:')||k.startsWith('webmaster.wm004-preview.v1:')));
    ...
    await page.screenshot({path:`${reportDir}/${fixture.name}-reopened.png`});results.push({fixture:fixture.name,status:'PASS',method:'Compiled normal-root release; genuine menu actions; exact old writer fixture; WM003 Replay 20-ring preserves earned S2/S3 progress; Save Game/Save & Quit; Chromium process closes and reopens same persistent profile without reinjection; identical stored generations and preserved root/WM003 keys',historicalLoaded,loaded,reopened,errors});
  }catch(error){
