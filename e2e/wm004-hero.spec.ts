...
async function shot(p:Page,name:string){await mkdir(out,{recursive:true});await p.screenshot({path:`${out}/${name}.png`});}
  ...
  for(const[name,alpha,key]of[["rear",-Math.PI/2,"w"],["front",Math.PI/2,"s"],["side",0,"d"]]as const)test(`WM004 hero ${name} idle walking running jump and stop transitions`,async({page})=>{
    ...
    try {
    const walking=page.evaluate(async key=>{const c=(window as any).__wm004Controls,states=[];c.keys(key);const start=performance.now();try{while(performance.now()-start<2000)states.push({...await c.frame(),recordedAt:performance.now()});}finally{c.keys();}await c.rest(700);return states;},key);
    ...
    const movement=await page.evaluate(async key=>{const c=(window as any).__wm004Controls,states=[];c.keys('Shift',key);const start=performance.now();try{while(performance.now()-start<300)states.push({...await c.frame(),recordedAt:performance.now()});c.keys('Shift',key,'Space');states.push({...await c.frame(),recordedAt:performance.now()});c.keys('Shift',key);while(performance.now()-start<600)states.push({...await c.frame(),recordedAt:performance.now()});}finally{c.keys();}await c.rest(1300);return states;},key);
