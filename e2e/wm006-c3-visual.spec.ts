...
test('WM006 continuous nine-attack observation, clear moving views and mute',async({page})=>{
 ...
 }finally{await writeFile(`${out}/observations.json`,JSON.stringify({source:process.env.WM_SOURCE,tree:process.env.WM_TREE,classification:'SELF_REVIEW sponsor follow-up source; reused C3 continuous-observation procedure; original C3 failed run remains historical',method:'Existing released browser-frame input sequencer and bounded native recorder. Observer installed before inputs, all nine active poses and three projectile variants asserted for each clear face-oriented view. No gameplay/pose/time changes. Perceived sound and physical devices remain unverified.',records,errors},null,2));}
});
