...
for (const [width, height] of [
  ...
  test(`parent FPS measurement ${width}x${height}`, async ({
    page,
    browserName,
  }) => {
    ...
    const samples = await page.evaluate(() =>
      (window as any).__WM_DEBUG__.performance(),
    );
    const output = process.env.WM_MEASUREMENT_OUTPUT ?? "evidence/wm-002";
    if (!output) throw new Error("WM_MEASUREMENT_OUTPUT required");
    await mkdir(output, { recursive: true });
    const record = {
      author: "WEBMASTER_IMPLEMENTER",
      label: process.env.WM_MEASUREMENT_LABEL ?? "candidate-isolated",
      ...
      samples,
      method:
        "Ordinary New Game / slot 1 / Easy; adaptive quality default; wait 2200ms after input-overlay; same debug.performance() samples as unchanged WM-001 representative gate; video enabled.",
      absoluteFloor: 30,
      absoluteResult: samples.minimum >= 30 ? "PASS" : "NOT_MET",
      limitation:
        "Headless Linux software-renderer measurement; not physical Windows/iPad or swing-route performance.",
    };
    await writeFile(
      `${output}/${record.label}-${width}x${height}.json`,
